...
namespace TMPro.EditorUtilities
{
    public class TMPCreater 
    {
        ...
        public static void ShowFontAtlasCreatorWindow(Font sourceFontFile,TextAsset asset)
        {
            var window = new TMPCreater();

            //window.ClearGeneratedData();
            window.m_LegacyFontAsset = null;
            window.m_SelectedFontAsset = null;
            window.m_CharactersFromFile = asset;
        ...
        private static MethodInfo _OverTryPack;
        private static Type _OverTryPackType;

        public static void CreateAssembly()
        {
            if (_OverTryPack == null)
            {
                var ass = Assembly.Load("UnityEngine");
                if (ass == null)
                    return;
                _OverTryPackType = ass.GetType("UnityEngine.TextCore.LowLevel.FontEngine", true, true);
            }
        }
        public static void OverTryPack(List<Glyph> glyphsToAdd,
            ...
            List<GlyphRect> usedGlyphRects)
        {
            ...
            {
                CreateAssembly();
                _OverTryPack = _OverTryPackType.GetMethod("TryPackGlyphsInAtlas", BindingFlags.NonPublic | BindingFlags.Static);
            }

            _OverTryPack.Invoke(_OverTryPackType, new object[] {glyphsToAdd, glyphsAdded, padding, packingMode, renderMode, width, height, freeGlyphRects, usedGlyphRects});
        }

        private static MethodInfo _OverRenderGlyphsToTexture;
        private static Type _OverRenderType;
        private static void OverRenderGlyphsToTexture(      List<Glyph> glyphs,
            int padding,
            GlyphRenderMode renderMode,
            Texture2D texture)
        {
            if (_OverRenderGlyphsToTexture == null)
            {
                CreateAssembly();
                _OverRenderGlyphsToTexture = _OverTryPackType.GetMethod("RenderGlyphsToTexture", BindingFlags.NonPublic | BindingFlags.Static);
            }
            _OverRenderGlyphsToTexture.Invoke(_OverRenderType, new object[] {glyphs, padding, renderMode, texture});
        }
        private static void OverRenderGlyphsToTexture(      List<Glyph> glyphs,
            int padding,
            GlyphRenderMode renderMode,
            ...
            int texHeight)
        {
            ...
            {
                CreateAssembly();
                var arr = new Type[] {typeof(List<Glyph>) ,typeof(int), typeof(GlyphRenderMode), typeof(byte[]), typeof(int), typeof(int) };
                var method = _OverTryPackType.GetMethod("RenderGlyphsToTexture", BindingFlags.NonPublic | BindingFlags.Static,null,arr,null );
                if (method != null)
                    _OverRenderGlyphsToTexture = method; //,BindingFlags.NonPublic | BindingFlags.Static
            }
            _OverRenderGlyphsToTexture.Invoke(_OverRenderType, new object[] {glyphs, padding, renderMode, texBuffer, texWidth, texHeight});
        }
        ...
        public int m_CharacterSetSelectionMode = 8 ;
        ...
        public TextAsset m_CharactersFromFile;
        public int m_PointSize = 30;
        int m_Padding = 9;
        //FaceStyles m_FontStyle = FaceStyles.Normal;
        //float m_FontStyleValue = 2;

        GlyphRenderMode m_GlyphRenderMode = GlyphRenderMode.SDFAA;
        public int m_AtlasWidth = 2048;
        public int m_AtlasHeight = 4096;
        ...
        {
            var method = _OverTryPackType.GetMethod("GetGlyphPairAdjustmentTable", BindingFlags.NonPublic | BindingFlags.Static);
            GlyphPairAdjustmentRecord[] adjustmentRecords = (GlyphPairAdjustmentRecord[])method.Invoke(_OverTryPackType,new object[]{m_AvailableGlyphsToAdd.ToArray()});
            ...
            var ass = Assembly.Load("UnityEngine");
            var t = typeof(TMP_GlyphAdjustmentRecord);
            
            var type = ass.GetType("UnityEngine.TextCore.LowLevel.FontEngine", true, true);
            var creater = _OverTryPackType.GetMethod("TMP_GlyphAdjustmentRecord");
            for (int i = 0; i < adjustmentRecords.Length && adjustmentRecords[i].firstAdjustmentRecord.glyphIndex != 0; i++)
            {
                var r = adjustmentRecords[i];
                var o1 = (TMP_GlyphAdjustmentRecord)Activator.CreateInstance(t, new object[] { r.firstAdjustmentRecord });
                var o2 = (TMP_GlyphAdjustmentRecord)Activator.CreateInstance(t, new object[] { r.secondAdjustmentRecord });
                var record = new TMP_GlyphPairAdjustmentRecord(o1,o2);
                fontFeatureTable.glyphPairAdjustmentRecords.Add(record);
            }
        ...
        public void Save(string font)
        {
            string filePath = Path.GetFullPath(font).Replace('\\', '/');

            Save_SDF_FontAsset(filePath);
        }
        public void SetValue(Type type,object obj,string name,object value)
        {
        ...
        void Save_SDF_FontAsset(string filePath)
        {
            ...
            var versionFI = fontType.GetProperty("version");
            ...
            versionFI.SetValue(fontAsset, "1.1.0");
            // var listInfo = fontType.GetProperty("m_glyphInfoList");
            // var list = (List<TMP_Glyph>)listInfo.GetValue(fontAsset, null);
            // // Special handling to remove legacy font asset data
            // if (list != null && list.Count > 0)
            //     list = null;

            //Set Font Asset Type
            SetValue(fontType,fontAsset,"atlasRenderMode",m_GlyphRenderMode);
            //fontAsset.atlasRenderMode = m_GlyphRenderMode;

            // Add FaceInfo to Font Asset
            SetValue(fontType,fontAsset,"faceInfo",m_FaceInfo);
            //fontAsset.faceInfo = m_FaceInfo;

            // Add GlyphInfo[] to Font Asset
            SetValue(fontType,fontAsset,"glyphTable",m_FontGlyphTable);
            //fontAsset.glyphTable = m_FontGlyphTable;

            // Add CharacterTable[] to font asset.
            SetValue(fontType,fontAsset,"characterTable",m_FontCharacterTable);
            //fontAsset.characterTable = m_FontCharacterTable;

            // Sort glyph and character tables.
            fontType.GetMethod("SortAllTables", BindingFlags.NonPublic | BindingFlags.Instance).Invoke(fontAsset, null);
            ...
            {
                SetValue(fontType,fontAsset,"fontFeatureTable",GetKerningTable());
            ...
            if (fontAsset.atlasTextures != null && fontAsset.atlasTextures.Length > 0)
            {
                for (int i = 1; i < fontAsset.atlasTextures.Length; i++)
                    GameObject.DestroyImmediate(fontAsset.atlasTextures[i], true);
            }
            SetValue(fontType,fontAsset,"m_AtlasTextureIndex",0);
            SetValue(fontType,fontAsset,"atlasWidth",m_AtlasWidth);
            SetValue(fontType,fontAsset,"atlasHeight",m_AtlasHeight);
            SetValue(fontType,fontAsset,"atlasPadding",m_Padding);
            ...
            if (ass == null)
                return;
            var typemodule = ass.GetType("UnityEditor.TextCore.LowLevel.FontEngineEditorUtilities", true, true);
            var method = typemodule.GetMethod("SetAtlasTextureIsReadable", BindingFlags.NonPublic | BindingFlags.Static);
            // Make texture readable to allow resizing
            bool isReadableState = tex.isReadable;
            if (isReadableState == false)
                method.Invoke(typemodule, new object[] { tex, true });
            ...
            //var typemodule = ass.GetType("UnityEditor.TextCore.LowLevel.FontEngineEditorUtilities", true, true);
            method = typemodule.GetMethod("SetAtlasTextureIsReadable", BindingFlags.NonPublic | BindingFlags.Static);
            method.Invoke(typemodule, new object[] { fontAsset.atlasTexture, false});
            //FontEngineEditorUtilities.SetAtlasTextureIsReadable(fontAsset.atlasTexture, false);

            // Add list of GlyphRects to font asset.
            SetValue(fontType,fontAsset,"freeGlyphRects",m_FreeGlyphRects);
            SetValue(fontType,fontAsset,"usedGlyphRects",m_UsedGlyphRects);
        ...
        public void Gen(Font font, string txt)
        {
            ...
            if (!string.IsNullOrEmpty(txt))
            {
                ...
                m_CharacterSequence = rx.Replace(txt,
                    match =>
                    {
            ...
            if (!m_IsProcessing && font != null)
                {
                    GameObject.DestroyImmediate(m_FontAtlasTexture);
                    GameObject.DestroyImmediate(m_GlyphRectPreviewTexture);
                    m_FontAtlasTexture = null;
                    ...
                    if (errorCode == FontEngineError.Success)
                    {
                        ...
                        if (errorCode != FontEngineError.Success)
                        {
                            Debug.Log("Font Asset Creator - Error Code [" + errorCode + "] has occurred trying to load the [" + font.name + "] font file. This typically results from the use of an incompatible or corrupted font file.", font);
                        }
                    }
                    ...
                    if (errorCode == FontEngineError.Success)
                    {
                            ...
                            if (m_AvailableGlyphsToAdd.Count > 0)
                            {
                                ...
                                if (m_PointSizeSamplingMode == 0) // Auto-Sizing Point Size Mode
                                {
                                    ...
                                    for (int iteration = 0; iteration < 15 && optimumPointSizeFound == false; iteration++)
                                    {
                                        ...
                                        OverTryPack(m_GlyphsToPack, m_GlyphsPacked, m_Padding, (GlyphPackingMode)m_PackingMode, m_GlyphRenderMode, m_AtlasWidth, m_AtlasHeight, m_FreeGlyphRects, m_UsedGlyphRects);
                        ...
                        if (m_IsGenerationCancelled == false)
                        {
                            ...
                            {
                                OverRenderGlyphsToTexture(m_GlyphsToRender, m_Padding, m_GlyphRenderMode, m_AtlasTextureBuffer, m_AtlasWidth, m_AtlasHeight);
                            }
                    ...
                    SaveCreationSettingsToEditorPrefs(SaveFontCreationSettings(font));
                }
        }
        ...
        public void CreateFontAtlasTexture()
        {
        ...
        // /// </summary>
        // /// <returns></returns>
        FontAssetCreationSettings SaveFontCreationSettings(Font font)
        {
            FontAssetCreationSettings settings = new FontAssetCreationSettings();
        
            //settings.sourceFontFileName = m_SourceFontFile.name;
            settings.sourceFontFileGUID = AssetDatabase.AssetPathToGUID(AssetDatabase.GetAssetPath(font));
